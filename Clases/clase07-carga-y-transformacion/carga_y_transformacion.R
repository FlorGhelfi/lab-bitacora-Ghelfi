...
ruta_o3 <- "/Users/floghelfi/Library/CloudStorage/OneDrive-UniversidaddeMontevideo/11AVO SEMESTRE/Introducción a la CD/GitHub/lab-bitacora-Ghelfi/Clases/clase07-carga-y-transformacion/o3_01_2024_04_2024.csv"
file.exists(ruta_o3)
ozono <- read_csv(ruta_o3, locale = locale(encoding = "latin1"))

## Material particulado menor de 2,5 micras, estaciones automáticas

pm <- read_csv(file.path(carpeta, "pm_2_5_01_2024_04_2024.csv"),
...
# FORMA ALTERNATIVA
ozono <- read_csv(ruta_o3, locale = locale(encoding = "UTF-8"))
unique(ozono$estacion)

# FORMA (de clase): Creamos el diccionario de traducción (Busca = Reemplaza)
pauta_limpieza <- c(
  "Ã³" = "o",
  "Ã±" = "ni",
  "Ã¡" = "á",
...
## 4.1 Agregación mensual - MUY IMPORTANTE
ozono_mes <- ozono_new %>%
  group_by(mes) %>%
  summarise(
...
#febrero tiene la media más alta y lamitad de los datos --> no es confiable, por eso tenemos la columan "n"
#en el gráfico lo vemos mejor
...
#Hago un chequeo extra para ferbrero
ozono_new %>%
  group_by(mes, estacion) %>%
  summarise(o3_medio = mean(o3), n = n(), .groups = "drop")
...
table(ozono_hora$n_min == 60)

#De las 4.312 horas que tenés, 4.165 están completas con sus 60 minutos y 147 no. O sea que cuando el sensor mide, mide bien: el 96,6% de las horas presentes están enteras.
#Faltan 1.496 horas — el 26%. Y no aparecen como incompletas ni como NA: directamente no existen como fila.
...
#me resultó dificil de interpretar este gráfico, voy a hacer otro
ozono_hora %>%
  ggplot(aes(x = fecha_hora, y = media)) +
  geom_line(linewidth = 0.3, color = "steelblue") +
  facet_wrap(~ estacion, ncol = 1) +
  theme_minimal() +
  labs(title = "Ozono horario por estación, ene–abr 2024",
       x = NULL, y = "O3 medio (µg/m³)")

grilla <- seq(min(ozono_hora$fecha_hora), max(ozono_hora$fecha_hora), by = "hour")

ozono_hora %>%
  complete(estacion, fecha_hora = grilla) %>%
  ggplot(aes(x = fecha_hora, y = media)) +
  geom_line(linewidth = 0.3, color = "steelblue") +
  facet_wrap(~ estacion, ncol = 1) +
  theme_minimal() +
...
source(file.path(carpeta, "estadistica_basica.R"))
print(pA)
print(pB)
...
# 9. Mapa
# =============================================================================
# install.packages("leaflet.minicharts")   # ya instalado
source(file.path(carpeta, "mapa.R"))
mapa
